...
<script src="${EJS_CDN}loader.js"><\/script>
  ...
  $effect(() => {
    ...
    const core = isCustomEmulator(game.platform)
      ? game.platform
      : getCoreForPlatform(game.platform);
    if (!core) {
    ...
    if (firstScreenshot) {
      resolveStorageUrl(firstScreenshot)
        .then((url) => {
          if (!cancelled) posterUrl = url;
        })
        .catch(() => {});
    }

    resolveStorageUrl(game.rom)
      .then((url) => {
        if (cancelled) return;
        loading = false;
        if (isZx81) {
  ...
  function launch() {
    ...
    else if (game.platform === "zxspectrum")
...
{#if loadError}
  <div
    class="flex aspect-video w-full max-w-4xl items-center justify-center rounded-lg bg-surface-800"
  >
    ...
    <div class="relative flex flex-col items-center gap-3">
      ...
      >
        <svg
          ...
          viewBox="0 0 24 24"
          fill="currentColor"
          class="h-10 w-10 translate-x-0.5 text-white"
        >
